...
> ### **Java 17 + SpringBoot 3 + MyBatis** codebase containing real world examples (CRUD, auth, advanced patterns, etc) that adheres to the [RealWorld](https://github.com/gothinkster/realworld) spec and API.

This codebase was created to demonstrate a fully fledged fullstack application built with **Java 17 + SrpingBoot 3 + MyBatis** including CRUD operations, authentication, routing, pagination, and more.

We've gone to great lengths to adhere to the **Java 17 + SrpingBoot 3 + MyBatis** community styleguides & best practices.
...
The application uses **Java 17 + SrpingBoot 3 + MyBatis**.
...
It uses a MariaDB database to store the application data, can initial the database by execute the schemal.sql under database.

# Getting started

You just need to have JDK 17 installed.

You just need to have MariaDB installed.

## Prepare database
Execute the database/schema.sql in MariaDB prompt
